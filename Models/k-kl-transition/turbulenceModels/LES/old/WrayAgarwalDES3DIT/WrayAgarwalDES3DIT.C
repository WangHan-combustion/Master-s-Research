...
tmp<volScalarField> WrayAgarwalDES3DIT<BasicTurbulenceModel>::chi() const
{
    return Rnu_/this->nu();
}

template<class BasicTurbulenceModel>
tmp<volScalarField> WrayAgarwalDES3DIT<BasicTurbulenceModel>::fv1
(
...
template<class BasicTurbulenceModel>
tmp<volScalarField> WrayAgarwalDES3DIT<BasicTurbulenceModel>::blend
(
...
template<class BasicTurbulenceModel>
void WrayAgarwalDES3DIT<BasicTurbulenceModel>::correctNut
(
...
template<class BasicTurbulenceModel>
tmp<volScalarField> WrayAgarwalDES3DIT<BasicTurbulenceModel>::fdes2
(
    const volScalarField& Lrans2r
) const
{
    tmp<volScalarField> Lles2(sqr(CDES_*this->delta()));
    //min(Lles2().dimensionedInternalField(), Lles2());
...
template<class BasicTurbulenceModel>
WrayAgarwalDES3DIT<BasicTurbulenceModel>::WrayAgarwalDES3DIT
(
...
template<class BasicTurbulenceModel>
bool WrayAgarwalDES3DIT<BasicTurbulenceModel>::read()
{
...
template<class BasicTurbulenceModel>
tmp<volScalarField> WrayAgarwalDES3DIT<BasicTurbulenceModel>::DRnuEff(volScalarField Switch) const
{
...
template<class BasicTurbulenceModel>
tmp<volScalarField> WrayAgarwalDES3DIT<BasicTurbulenceModel>::k() const
{
    return tmp<volScalarField>
    (
        new volScalarField
...
template<class BasicTurbulenceModel>
tmp<volScalarField> WrayAgarwalDES3DIT<BasicTurbulenceModel>::epsilon() const
{
    WarningInFunction
...
template<class BasicTurbulenceModel>
void WrayAgarwalDES3DIT<BasicTurbulenceModel>::correct()
{
    if (!this->turbulence_)
    {
        return;
...
////////////////////////////////////////////////////////////////
    ...
    /*
	// Calculate length scales and fdes
    // fdes = 1 in RANS regino, fdes = 0 in LES region
    volScalarField Lrans2r(magSqr(fvc::grad(S))/S2);
    volScalarField fdes2(this->fdes2(Lrans2r));
    fdes_= sqrt(fdes2);
    bound(fdes2,SMALL);
    
    // Blend Scheme
    // Use the first scheme if blendfactor_ = 1, use the second secheme if blendfactor_ = 0
    ...
    */
    volScalarField Lles2r(1.0/sqr(CDES_*this->delta()));
    volScalarField Lrans2r(magSqr(fvc::grad(S))/S2);
    //volScalarField Ldes2r(max(Lles2r,Lrans2r));
    volScalarField Ldes2r(Lles2r);
    
    // fdes = 1 in RANS regino, fdes = 0 in LES region
    fdes_ = neg(Lles2r - Ldes2r);
...
////////////////////////////////////////////////////////////////
    ...
    (
     ...
     ==
	  ...
	  + C2kw_*Switch1_*alpha*rho*fvm::Sp((fvc::grad(Rnu_) & fvc::grad(S))/S,Rnu_)
	  - (1.0-Switch1_)*alpha*rho*fvm::Sp((C2ke_)*Rnu_*Ldes2r,Rnu_)
    );
...
template<class BasicTurbulenceModel>
tmp<volScalarField> WrayAgarwalDES3DIT<BasicTurbulenceModel>::LESRegion() const
{
