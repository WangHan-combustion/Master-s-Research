...
) const
{
  ...
  volScalarField dw = max(mag(y_), dimensionedScalar("1e-6", dimensionSet(0, 1, 0, 0, 0), 1e-15));
    ...
    //max(min(scalar(100)*(sqrt(Rnu_/scalar(1.5)))/(sqrt(omeg)*dw), scalar(100.0)), scalar(0));
    //return max(min(scalar(100)*(sqrt(Rnu_*omeg/scalar(1.5)))/(UU), scalar(100.0)), scalar(0.0001));
}
...
template<class BasicTurbulenceModel>
tmp<volScalarField> GReWA<BasicTurbulenceModel>::Fonset1
(
...
template<class BasicTurbulenceModel>
tmp<volScalarField> GReWA<BasicTurbulenceModel>::Fonset2
(
...
template<class BasicTurbulenceModel>
tmp<volScalarField> GReWA<BasicTurbulenceModel>::Fonset3
(
    const volScalarField& Rt  
) const
{
    return max(1.0-pow3(Rt/3.5), scalar(0.0));
}

template<class BasicTurbulenceModel>
tmp<volScalarField> GReWA<BasicTurbulenceModel>::Fonset
(
...
:
    ...
    (
        ...
        (
            "C1kw",
            this->coeffDict_,
			0.0833 // 0.0829 doesnt change too much
        )
    ),
...
void GReWA<BasicTurbulenceModel>::correct()
{
    ...
    const volScalarField Fonset(this->Fonset(Fonset1, Fonset3));
