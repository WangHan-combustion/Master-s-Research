...
tmp<volScalarField> WrayAgarwalWR<BasicTurbulenceModel>::chi() const
{

    return Rnu_/(this->nu())+Cr1_*ks_/(y_+0.03*ks_);

}

template<class BasicTurbulenceModel>
tmp<volScalarField> WrayAgarwalWR<BasicTurbulenceModel>::fmu
(
...
template<class BasicTurbulenceModel>
tmp<volScalarField> WrayAgarwalWR<BasicTurbulenceModel>::blend
(
...
template<class BasicTurbulenceModel>
tmp<volScalarField> WrayAgarwalWR<BasicTurbulenceModel>::sigmaR
(
    const volScalarField& Switch
) const
{
...
template<class BasicTurbulenceModel>
tmp<volScalarField> WrayAgarwalWR<BasicTurbulenceModel>::C1
(
    const volScalarField& Switch
) const
{
...
template<class BasicTurbulenceModel>
void WrayAgarwalWR<BasicTurbulenceModel>::correctNut
(
...
template<class BasicTurbulenceModel>
WrayAgarwalWR<BasicTurbulenceModel>::WrayAgarwalWR
(
...
template<class BasicTurbulenceModel>
bool WrayAgarwalWR<BasicTurbulenceModel>::read()
{
...
template<class BasicTurbulenceModel>
tmp<volScalarField> WrayAgarwalWR<BasicTurbulenceModel>::DRnuEff(volScalarField Switch) const
{
...
template<class BasicTurbulenceModel>
tmp<volScalarField> WrayAgarwalWR<BasicTurbulenceModel>::k() const
{
    return tmp<volScalarField>
    (
        new volScalarField
...
template<class BasicTurbulenceModel>
tmp<volScalarField> WrayAgarwalWR<BasicTurbulenceModel>::epsilon() const
{
    return tmp<volScalarField>
    (
        new volScalarField
...
template<class BasicTurbulenceModel>
void WrayAgarwalWR<BasicTurbulenceModel>::correct()
{
    if (!this->turbulence_)
    {
        return;
...
/*
 volScalarField eta = y_+0.03*ks_*sqrt(Rnu_*S)/(20.0*this->nu());
    f1_ = tanh(pow((1.0+20.0*eta)/(1.0+sqr(max(y_+0.03*ks_*sqrt(Rnu_*S),Am_*Rnu_)/(20.0*this->nu()))),4.0));    
    f1_ = min(f1_,0.9);
    bound(f1_,SMALL);
*/
//ks_
 volScalarField eta = (y_+0.03*ks_) *  sqrt(Rnu_*S)
			             /
			             (20.0*this->nu());
    f1_ = tanh(
                pow(
                    ...
                    /
                    (1.0+sqr(
                               max((y_+0.03*ks_)*sqrt(Rnu_*S),Am_*Rnu_)
                               /
                               (20.0*this->nu())
                            )
                    ),
...
*/
    ...
    (
     ...
     ==
        alpha*rho*C1(f1_)*fvm::Sp(S, Rnu_)
      + alpha*rho*f1_*C2kw_*(y_/(y_+0.03*ks_))              
      * fvm::Sp((fvc::grad(Rnu_)&fvc::grad(S))/S, Rnu_)
      - alpha*rho*(1.0-f1_)*min(C2ke_*Rnu_*Rnu_*magSqr(fvc::grad(S))/S2,
                                Cm_*magSqr(fvc::grad(Rnu_)))
    );
    RnuEqn().relax();
...
/*
    ...
    forAll(patches, patchi)
    {
      ...
      Rnu_[faceCelli] = Rnutemp[faceCelli]*0.03*ks_.value()
                /
            (y_+0.03*ks_[faceCelli]+0.03*ks_.value());
...
/*
    ...
    Rnu_[faceCelli]  = gradnR[faceCelli]
                        *
                        0.03*ks_.value();
