...
{
    return Rnu_/this->nu();
    //return Rnu_/this->nu()+Cr1_*(1+0.03*ks_);
}
...
/*
	forAll(patches, patchi)
	{
		...
		if (isType<wallFvPatch>(curPatch))
		{
			...
			//const scalarField& nuw= this->nu().boundaryField()[patchi];
			//const volScalarField& nutw = this->nut_.boundaryField()[patchi];
			//const volScalarField Rw = Rnu_.boundaryField()[patchi];
			//const scalarField& utw = utau_.boundaryField()[patchi];
			scalarField magFaceGradU= mag(this->U_.boundaryField()[patchi].snGrad());
			...
			forAll(curPatch, facei)
			{
				...
				Rnu_.boundaryField()[faceCelli] = utau_.boundaryField()[patchi]
								  /
						 		  (0.41*0.03*ks_.value());
	...
	const fvPatchList& patches = this->mesh_.boundary();
		...
		if (isType<wallFvPatch>(currPatch))
		{
			...
			utau_.boundaryField()[patchi] =sqrt(this->nu()*mag(wGUx_.boundaryField()[patchi]));
	 	        ...
	 	        this->nut_[faceCelli]  = utau_[faceCelli]
					   /
					   (0.41*0.03*ks_.value());
